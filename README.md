...
```
Crea el diagrama de secuencia para la estructura que diseñaste.
...
sequenceDiagram
    ...
    Main->>Materia: new Materia("A","a1234",7,20)
    Main->>Materia: new Materia("B","a4321",6,15)
    Main->>Materia: new Materia("C","a2134",5,17)

    %% Creación del curso
    Main->>Curso: new Curso("Semestre 1", m1, m2, m3)
    Curso->>Materia: copiar datos de materias

    %% Creación del alumno
    Main->>Alumno: new Alumno("Yo","AL12345678",19, curso)

    %% Creación del profesor
    Main->>Profesor: new Profesor("Profe","1234",500,m1)

    %% Mostrar datos del curso
    Main->>Curso: getNombre()
    Curso-->>Main: nombre

    Main->>Curso: calcularCreditosCurso()
    Curso->>Materia: getCreditos()
    Materia-->>Curso: creditos
    Curso-->>Main: totalCreditos

    %% Mostrar datos del alumno
    Main->>Alumno: getNombre()
    Alumno-->>Main: nombre

    Main->>Alumno: getCurso()
    Alumno-->>Main: curso

    %% Mostrar datos del profesor
    Main->>Profesor: getMateria()
    Profesor-->>Main: materia
